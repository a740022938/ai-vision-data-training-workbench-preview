...
class MainWindow:
    def __init__(self, root):
        ...
        self.drag_mode = None   # add / move / resize
        self.dragging = False
        self.start_x = 0
        self.start_y = 0
        self.temp_x = 0
    ...
    def show_settings_tip(self):
        messagebox.showinfo("提示", "设置窗口下一步再正式接入，这一版先把核心标注功能整合好。")
    ...
    def load_images(self):
        ...
        self.project_info_label.config(
            text=(
                f"图片目录：{self.image_dir or '未设置'}\n"
                f"标签目录：{self.label_dir or '未设置'}\n"
                f"图片数量：{len(self.image_files)}"
            )
        )
    ...
    def show_current_image(self):
        ...
        try:
            ...
            self.image_info_label.config(
                text=f"{self.current_image_name}  [{self.current_index + 1}/{len(self.image_files)}]"
            )
    ...
    def redraw_boxes(self, temp_rect=None):
        ...
        for i, box in enumerate(self.boxes):
            ...
            self.image_canvas.create_rectangle(
                x1 + self.img_offset_x,
                y1 + self.img_offset_y,
                x2 + self.img_offset_x,
                y2 + self.img_offset_y,
                outline=color,
                width=width,
                tags="box"
            )
