...
class MainWindow:
    ...
    def not_ready_yolo(self):
        ...
        result = send_context_to_external(context)
        messagebox.showinfo("提示", result.get("message", "外部接口已调用"))
    ...
    def on_right_click(self, event):
        ...
        menu.add_command(label="编辑当前框", command=self.open_edit_window)
        menu.add_command(label="删除当前框", command=self.delete_selected_box)
        menu.add_separator()
